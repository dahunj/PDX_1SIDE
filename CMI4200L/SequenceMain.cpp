...
// 				if (gData.nPortNo == 0) break;	// Cap Attach에서 Port No를 받는다.
	...
	case 250:
		if (m_nLoad1Case==100 ||(m_nLoad1Case >=360 && m_nLoad1Case < 480))
			m_nLoad2Case = 260;
			m_pCommon->Set_LoopTime(AUTO_LOAD2, 5000);
		break;
	case 260:
	...
	case 270:
		if((gData.NGPicNo==1 && m_pDX3->iNGPicker1Up && !m_pDX3->iNGPicker1Down) ||
		   ...
		   (gData.NGPicNo==5 && m_pDX3->iNGPicker5Up && !m_pDX3->iNGPicker5Down) ) {
			...
			if (gData.PickerInfor[1][gData.NGPicNo-1] >= 2) gLot.nNGCnt++;
			if (gData.PickerInfor[1][gData.NGPicNo-1] == 9) gLot.nBoNGCnt++;
...
BOOL CSequenceMain::Check_NGTrayXPickDownSelect()
{
	...
	for(int l=0; l<gData.nPickCnt; l++) {
		if (gData.PickerInfor[1][l] > 0) {
			gData.NGPicNo = l + 1;
			if (gData.PickerInfor[1][l] == 9) gData.nNGTrayPos = 1;	//Barrel 오염
