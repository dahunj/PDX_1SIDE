...
BEGIN_MESSAGE_MAP(CWorkDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHK_ALL_PASS_FUNCTION, &CWorkDlg::OnBnClickedChkAllPassFunction)
...
//	for (int i = 39; i < 48; i++) m_Label[i].Init_Ctrl("바탕", 11, FALSE, COLOR_DEFAULT, COLOR_DEFAULT);
	...
	m_Label[10].Init_Ctrl("바탕", 10, FALSE, RGB(0xFF, 0xFF, 0xFF), RGB(0x40, 0x00, 0x80));
	m_ledBarVision.Init_Ctrl("바탕", 11, FALSE, COLOR_DEFAULT, COLOR_DEFAULT, CLedCS::emGreen, CLedCS::em16);
...
void CWorkDlg::OnBnClickedChkAllPassFunction()
{
	CSequenceMain *pSequenceMain = CSequenceMain::Get_Instance();
	CCommon *pCommon = CCommon::Get_Instance();
	CDataManager *pDataManager = CDataManager::Get_Instance();
